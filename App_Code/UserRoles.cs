...
public class UserRoles
{
    ...
    public static List<UserRoles> GetUserRoles(SqlConnection conAP, string cat)
    {
        List<UserRoles> result = new List<UserRoles>();
        try
        {
            string cmdText = "Select * from UserRoles where Status!='Deleted' and RoleName=@RoleName";
            using (SqlCommand sqlCommand = new SqlCommand(cmdText, conAP))
            {
                sqlCommand.Parameters.AddWithValue("@RoleName", SqlDbType.NVarChar).Value = cat;
        ...
        {
            ExceptionCapture.CaptureException(HttpContext.Current.Request.Url.PathAndQuery, "GetUserRoles", ex.Message);
        }
