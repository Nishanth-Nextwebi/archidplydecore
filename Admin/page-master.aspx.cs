...
public partial class Admin_page_master : System.Web.UI.Page
{
    ...
    public void BindPageGroup()
    {
        try
        {
            List<PageGroup> pg = PageGroup.GetAllPageGroup(conAP);
            if (pg.Count > 0)
            {
    ...
    public void GetPageMaster()
    {
        try
        {
            List<PageMaster> comps = PageMaster.GetAllPageMaster(conAP).Where(x => x.Id == Convert.ToInt32(Request.QueryString["id"])).ToList();
            if (comps.Count > 0)
            {
                btnSave.Text = "Update";
        ...
        try
        {
            if (Page.IsValid)
            {
                ...
                if (btnSave.Text == "Update")
                {
                    ...
                    ca.Status = "Active";
                    int result = PageMaster.UpdatePageMaster(conAP, ca);
                    if (result > 0)
                    {
                        GetAllPageMaster();
                        GetPageMaster();
                ...
                else
                {
                    ...
                    ca.UpdatedOn = TimeStamps.UTCTime();
                    ca.Status = "Active";
                    int result = PageMaster.InsertPageMaster(conAP, ca);
                    if (result > 0)
                    {
                        GetAllPageMaster();
    ...
    public void GetAllPageMaster()
    {
        ...
        {
            strPage = "";
            List<PageMaster> cas = PageMaster.GetAllPageMaster(conAP).OrderByDescending(s => Convert.ToDateTime(s.UpdatedOn)).ToList();
            int i = 0;
...
<td><a href='javascript:void();' class='bs-tooltip' data-id='" + ca.Id + @"' data-bs-toggle='tooltip' data-placement='top' title=''>" + ca.UpdatedOn.ToString("dd-MMM-yyyy") + @"</a></td>  
    ...
    public static string Delete(string id)
    {
        ...
        try
        {
            ...
            int exec = PageMaster.DeletePageMaster(conAP, cat);
            if (exec > 0)
            {
                x = "Success";
            }
