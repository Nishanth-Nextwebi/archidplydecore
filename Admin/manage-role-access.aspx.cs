...
public partial class Admin_manage_role_access : System.Web.UI.Page
{
    ...
    public void GetUserAccess()
    {
        try
        {
            List<UserRolesAccess> users = UserRoles.GetAllUserRoles(conAP, Request.QueryString["id"]).OrderBy(x => x.GOrderBy).ToList();
            int i = 0;
...
<div class='form-check form-switch changeAccess' id='v_" + rl.PageId + @"'  data-id='" + rl.PageId + @"'>
    ...
    public static string ChangeStatus(string pid, string rid, string v, string a, string e, string d)
    {
        ...
        try
        {
            ...
            SqlConnection conAP = new SqlConnection(ConfigurationManager.ConnectionStrings["conAP"].ConnectionString);
            List<UserRolesAccess> users = UserRoles.GetAllUserRolesCheck(conAP, rid, pid);
            UserRolesAccess ua = new UserRolesAccess();
            if (users.Count > 0)
            {
                if (CreateUser.CheckAccess(conAP, "manage-role-access.aspx", "Edit", HttpContext.Current.Request.Cookies["ap_aid"].Value))
                {
                    ua.PageId = pid;
                    ua.RoleId = rid;
                    ...
                    int exec = UserRoles.UpdateRoleAccess(conAP, ua);
                    if (exec > 0)
                    {
                        x = "Success";
                    }
            ...
            {
                if (CreateUser.CheckAccess(conAP, "manage-role-access.aspx", "Add", HttpContext.Current.Request.Cookies["ap_aid"].Value))
                {
                    ua.PageId = pid;
                    ua.RoleId = rid;
                    ua.AddAccess = a == "Y" ? "1" : "0";
                    ...
                    ua.Status = "Active";
                    ua.PageGroupId = "";
                    int exec = UserRoles.InsertToRoleAccess(conAP, ua);
                    if (exec > 0)
                    {
                        x = "Success";
                    }
