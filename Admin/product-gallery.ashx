<%@ WebHandler Language="C#" Class="product_gallery" %>
...
public class product_gallery : IHttpHandler
{
    public void ProcessRequest(HttpContext context)
    {
        if (context.Request.Files.Count > 0)
        {
            ...
            if (!CreateUser.CheckAccess(conAP, "add-Product.aspx", "Add", aid))
            {
            ...
            for (int i = 0; i < files.Count; i++)
            {
                ...
                string iconPath = context.Server.MapPath(".") + "\\../UploadImages\\" + ImageGuid1 + "_product-gallery" + fileExtension;
                if ((fileExtension == ".jpg" || fileExtension == ".jpeg" || fileExtension == ".png" || fileExtension == ".webp" || fileExtension == ".gif"))
                {
                    ...
                    string bImage = "UploadImages/" + ImageGuid1 + "_product-gallery" + fileExtension;

                    ProductGallery pi = new ProductGallery();

                    pi.ProductId = Convert.ToString(strPid);
                    pi.GType = strType;
                    ...
                    x += ProductGallery.InsertProductGallery(conAP, pi);
                }
            }
            if (x > 0)
            {
