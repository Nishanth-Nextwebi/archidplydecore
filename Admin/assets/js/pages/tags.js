$(document).ready(function () {
    $(".txtTag").change(function () {
        $(".txtUrl").val($(".txtTag").val().toLowerCase().replace(/\./g, '').replace(/\//g, '').replace(/\&/g, 'and').replace(/\\/g, '').replace(/\*/g, '').replace(/\?/g, '').replace(/\~/g, '').replace(/\ /g, '-'));
    });
    $(document).on("keypress", function (e) {
        if (e.shiftKey) {
            return;
        }
        if (e.which == 13) { document.getElementById('[id*=btnSave]').click(); }
    });
                ...
                $.ajax({
                    ...
                    success: function (data2) {
                        if (data2.d.toString() == "Success") {
                            Swal.fire({ title: "Deleted!", text: "Tag has been deleted.", icon: "success", confirmButtonClass: "btn btn-primary w-xs mt-2", buttonsStyling: false })
