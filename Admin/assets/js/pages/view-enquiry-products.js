$(document).ready(function () {
    $(document.body).on('click', '.deleteItem', function () {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: 'POST',
                    url: "view-enquiry-products.aspx/Delete",
                    data: "{id: '" + id + "'}",
                    ...
                    success: function (data2) {
                        if (data2.d.toString() == "Success") {
                            Swal.fire({ title: "Deleted!", text: "Product has been deleted.", icon: "success", confirmButtonClass: "btn btn-primary w-xs mt-2", buttonsStyling: false })
    ...
    $('.publishProduct').on('change', function () {
        var id = $(this).attr('data-id');
        var ftr = $(this).prop("checked") ? "Yes" : "No";
        $.ajax({
            type: 'POST',
            url: "view-enquiry-products.aspx/PublishProduct",
            data: "{id: '" + id + "',ftr: '" + ftr + "'}",
            contentType: 'application/json; charset=utf-8',
            dataType: "json",
            async: false,
            success: function (data2) {
                if (data2.d.toString() == "Success") {
                    ...
                    if (ftr === "Yes") {
                        ...
                        Snackbar.show({
                            pos: 'top-right',
                            text: 'Product Drafted successfully',
                            actionTextColor: '#fff',
