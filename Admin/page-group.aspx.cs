...
public partial class Admin_page_group : System.Web.UI.Page
{
    ...
    public void GetGroups()
    {
        try
        {
            List<PageGroup> comps = PageGroup.GetAllPageGroup(conAP).Where(x => x.Id == Convert.ToInt32(Request.QueryString["id"])).ToList();
            if (comps.Count > 0)
            {
                btnSave.Text = "Update";
        ...
        try
        {
            if (Page.IsValid)
            {
                ...
                if (btnSave.Text == "Update")
                {
                    ...
                    ca.Status = "Active";
                    int result = PageGroup.UpdatePageGroup(conAP, ca);
                    if (result > 0)
                    {
                        GetAllGroup();
                        GetGroups();
                ...
                else
                {
                    ...
                    ca.UpdatedOn = TimeStamps.UTCTime();
                    ca.Status = "Active";
                    int result = PageGroup.InsertPageGroup(conAP, ca);
                    if (result > 0)
                    {
    ...
    public void GetAllGroup()
    {
        ...
        {
            strPageGroups = "";
            List<PageGroup> cas = PageGroup.GetAllPageGroup(conAP).OrderByDescending(s => Convert.ToDateTime(s.UpdatedOn)).ToList();
            int i = 0;
...
<td><a href='javascript:void();' class='bs-tooltip' data-id='" + ca.Id + @"' data-bs-toggle='tooltip' data-bs-placement='top' title=''>" + ca.UpdatedOn.ToString("dd-MMM-yyyy") + @"</a></td>  
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conAP = new SqlConnection(ConfigurationManager.ConnectionStrings["conAP"].ConnectionString);
            if (CreateUser.CheckAccess(conAP, "page-group.aspx", "Delete", HttpContext.Current.Request.Cookies["ap_aid"].Value))
            {
                PageGroup cat = new PageGroup();
                cat.Id = Convert.ToInt32(id);
                cat.UpdatedOn = TimeStamps.UTCTime();
                int exec = PageGroup.DeletePageGroup(conAP, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
